...
export default function LancamentoRow({ lancamento, categoria, onStatusChange, onClick }) {
  ...
  return (
    <div
      ...
      className="flex items-center justify-between gap-3 px-4 py-3 bg-white dark:bg-ink-700 rounded-card shadow-card
    ...
    >
      <div className="flex items-center gap-3 min-w-0 flex-1">
        ...
        <div className="min-w-0 flex-1">
          <p className="text-sm font-medium text-ink-900 dark:text-ink-50 flex items-center gap-1.5 min-w-0">
          ...
          {lancamento.parcelamentoId && (
            <p className="flex items-center gap-1 text-xs text-clay-600 mt-0.5">
      ...
      <div className="flex flex-col items-end gap-1 shrink-0">
        <span className={`money text-sm font-semibold whitespace-nowrap ${isReceita ? 'text-ledger-600' : 'text-ink-900 dark:text-ink-50'}`}>
