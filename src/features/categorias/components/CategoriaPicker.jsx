...
export default function CategoriaPicker({ categorias, value, onChange }) {
  ...
  return (
    <div ref={rootRef} onKeyDown={handleKeyDown} className="relative">
      ...
      {open && (
        <div className="absolute z-40 mt-1.5 w-full max-h-72 overflow-y-auto rounded-xl border border-ink-100 bg-white shadow-card-hover p-3">
            ...
            <button type="button" onClick={() => pick('')} className="flex flex-col items-center gap-1">
              <span
                className={`w-12 h-12 rounded-full flex items-center justify-center border-2 border-dashed transition-colors ${
                  !value ? 'border-ink-900 bg-ink-50' : 'border-ink-100 text-ink-300 hover:border-ink-300'
                }`}
              >
                <Ban size={18} strokeWidth={1.75} className={!value ? 'text-ink-900' : 'text-ink-300'} />
              </span>
