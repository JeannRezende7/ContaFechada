...
export default function PeriodNav({ periodType, anchor, customRange, onChangePeriodType, onChangeAnchor, onChangeCustomRange }) {
  return (
    <div className="mb-4">
      <div className="flex gap-1 bg-ink-50 rounded-pill p-1 mb-2">
        {PERIOD_TYPES.map((opt) => (
          <button
            ...
            className={`flex-1 rounded-pill py-1.5 text-xs font-medium transition-colors ${
              periodType === opt.key ? 'bg-white shadow-card text-ink-900' : 'text-ink-500'
            }`}
          >
            {opt.label}
          </button>
      ...
      {periodType === 'periodo' ? (
        <div className="flex items-center gap-2 bg-white rounded-card shadow-card px-3 py-2">
          <input
            type="date"
        ...
        <div className="flex items-center justify-between bg-white rounded-card shadow-card px-2 py-2">
          <button
            type="button"
          ...
          <span className="flex items-center gap-2 font-display text-sm font-semibold text-ink-900 text-center">
